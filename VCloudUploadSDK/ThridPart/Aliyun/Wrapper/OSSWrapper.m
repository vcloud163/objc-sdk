...
- (void)asyncGetImage:(NSString *)objectKey 
    ...
    _normalDloadRequest.bucketName = self.bucketName;
...
- (void)asyncPutImage:(NSString *)objectKey 
    ...
    _normalUploadRequest.bucketName = self.bucketName;
    ...
    _normalUploadRequest.uploadProgress = ^(int64_t bytesSent, int64_t totalByteSent, int64_t totalBytesExpectedToSend) {
        ...
        if (option && option.progressHandler) {
            option.progressHandler(key, progress);
        };
    };
    
    // 将 Base64 编码的字符串转换为 NSData
    NSData *decodedData = [[NSData alloc] initWithBase64EncodedString:self.callback options:0];
    NSError *error;
    ...
    if (jsonObject && [jsonObject isKindOfClass:[NSDictionary class]]) {
        ...
        callbackBodyType = [decodedDictionary objectForKey:@"callbackBodyType"];
...
- (void)multipartUploadWithSuccess:(void (^)(id _Nonnull))success failure:(void (^)(NSError * _Nonnull))failure {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        resumableUpload.bucketName = self.bucketName;            // 设置bucket名称
