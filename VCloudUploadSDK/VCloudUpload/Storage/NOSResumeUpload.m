...
- (instancetype)initWithData:(NSData *)data
	...
	if (self = [super init]) {
        ...
        _headers[@"Host"] = @"nosup-hz1.127.net";
...
- (void)record:(UInt32)offset {
	...
	if (error != nil) {
		DLog(@"up record json error %@ %@", key, error);
		return;
	}
	error = [_recorder set:key data:data];
	if (error != nil) {
		DLog(@"up record set error %@ %@", key, error);
	}
}
...
- (UInt32)recoveryFromRecord {
	...
	if (t != _modifyTime) {
		DLog(@"modify time changed %llu, %llu", t, _modifyTime);
		return 0;
	}
    _context = context;
	return offset;
...
- (void)nextTask:(UInt32)offset
    retriedTimes:(NSInteger)retried
    ...
    NOSCompleteBlock completionHandler = ^(NOSResponseInfo *info, NSDictionary *resp) {
		...
		if (info.error != nil) {
            NSInteger nextIndex = statItems.uploaderIndex + 1;
            NSInteger nextRetried = retried + 1;
